...
var UserSchema = new Schema({
    ...
    //Role
    role: {type: String, default: userRoles.public},
...
module.exports = {
    ...
    validate: function(username, email, role, password) {
        var deferred = Q.defer();

        var check = true;
        var msg = null;

        if (!validator.isLength(username, 1, 20)) {check = false; msg = 'Username must be between 1-20 characters long'}
        if (!validator.isLength(password, 5, 60)) {check = false; msg = 'Password must be between 5-60 characters long'}
        if (!validator.isEmail(email)) {check = false; msg = 'Invalid email address'}
        if (!validator.isAlphanumeric(username)) {check = false; msg = 'Username can only contain letters and numbers'}

        var stringArr = _.map(_.values(userRoles), function(val) { return val.toString() });
        if (!validator.isIn(role, stringArr)) {check = false; msg='Invalid role type'}
    ...
    addUser: function(username, email, role, provider, providerId, password) {
        var deferred = Q.defer();
        var user = null;
        ...
        if (password === null) {
            ...
            this.validate(username, email, role, password).then(function() {
